...
SurrealPilot is a hybrid Laravel application with:
- **Web SaaS**: React/Inertia frontend with Laravel backend
- **Desktop App**: NativePHP/Electron desktop distribution
- **Unreal Engine Plugin**: C++/Blueprint integration
- **MCP Servers**: Model Context Protocol for AI integration

Key components:
- `app/`: Laravel application (Models, Controllers, Services)
- `resources/`: React frontend with Inertia
- `UnrealEngine/`: UE plugin source code
...
composer run native:dev      # Desktop app development
...
npm run test:playcanvas     # PlayCanvas MCP tests
npm run test:mcp:all        # All MCP comprehensive tests
```
...
- **PHPUnit**: `tests/Unit/`, `tests/Feature/`, `tests/Browser/`
- **Jest**: JavaScript component tests
- **Puppeteer**: Headless browser testing (MCP servers)
- **Dusk**: Laravel browser testing

## Key Configuration Files

- `.env`: Environment variables (AI providers, Stripe, database)
...
Uses Vizra ADK with multi-provider support:
- OpenAI, Anthropic, Gemini, Ollama
- Configured via `.env` with API keys
- Streaming responses with Server-Sent Events
- Credit-based usage system

## Deployment

Production deployment uses Laravel Forge with scripts in `scripts/`:
- `setup-forge.sh`: Server setup
- `deploy-production.sh`: Application deployment
- `post-deploy.sh`: Post-deployment tasks

Desktop apps built via GitHub Actions (`.github/workflows/desktop-release.yml`)
